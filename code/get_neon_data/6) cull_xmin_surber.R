...
# 1) load data
dat_2024_surber = readRDS(file = "data/dat_2024_surber.rds")
...
# 5) bind xmins
xmins_clauset_surber = bind_rows(xmin_list)
saveRDS(xmins_clauset_surber, file = "data/xmins_clauset_surber_surber.rds")
...
dat_2024_clauset_xmins_surber = dat_2024_surber %>% 
  left_join(xmins_clauset_surber) %>% 
  group_by(sample_id) %>% 
...
dat_2024_clauset_xmins_surber %>% 
  group_by(sample_id, xmin) %>% 
  tally() %>% 
  arrange(n)

resample = dat_2024_clauset_xmins_surber %>% 
  group_by(sample_id) %>% 
  add_tally() %>% 
