...
macro_dw_sizeonly_temp = readRDS(file = "data/macro_dw_sizeonly_surber.rds") %>% mutate(taxon = "macro",
...
macro_fish_raw_surber = data2 %>% 
  filter(sample_id %in% ids_to_keep$sample_id) %>% 
  group_by(site_id, dw, sample_id) %>% 
  mutate(collect_date == min(collect_date))

saveRDS(macro_fish_raw_surber, file = "data/macro_fish_raw_surber.rds")

dat_2024_surber = macro_fish_raw_surber %>% 
  filter(sample_id %in% ids_to_keep$sample_id) %>% 
  group_by(site_id, dw, sample_id) %>% 
  mutate(collect_date = min(collect_date)) %>% 
...
saveRDS(dat_2024_surber, file = "data/dat_2024_surber.rds")

dat_2024_macros_surber = macro_dw_sizeonly_temp %>% 
  group_by(site_id, dw, macro_id) %>% 
  mutate(collect_date == min(collect_date)) %>%
  group_by(site_id, collect_date, dw, macro_id) %>% 
  reframe(no_m2 = mean(no_m2))

dat_2024_fish_surber = fish %>% 
...
saveRDS(dat_2024_macros_surber, file = "data/dat_2024_macros.rds")
saveRDS(dat_2024_fish_surber, file = "data/dat_2024_fish.rds")
