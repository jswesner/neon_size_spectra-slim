...
(interaction_plot_macros = int_plot_data %>% 
    ...
    labs(y = "\u03bb (ISD exponent)",
         ...
         subtitle = "Macroinvertebrates only") +
    theme(text = element_text(size = 7)) 
)


# ggview::ggview(interaction_plot, width = 4, height = 4)
ggsave(interaction_plot_macros, width = 4, height = 4,
       file = "plots/interaction_plot_macros.jpg")
saveRDS(interaction_plot_macros,  file = "plots/interaction_plot_macros.rds")
...
#4) make interaction plot showing lambda across different values of temp, om, and gpp. Then save it
(interaction_plot_fish = int_plot_data %>% 
    mutate(fill_color = case_when(quantile_om == "Low OM" ~ -0.25,
                                  quantile_om == "High OM" ~ -0.75,
                                  TRUE ~ -0.5),
           quantile_gpp = fct_relevel(quantile_gpp, "Low GPP", "Median GPP"),
...
# ggview::ggview(interaction_plot, width = 4, height = 4)
ggsave(interaction_plot_fish, width = 4, height = 4,
       file = "plots/interaction_plot_fish.jpg")
saveRDS(interaction_plot_fish,  file = "plots/interaction_plot_fish.rds")
